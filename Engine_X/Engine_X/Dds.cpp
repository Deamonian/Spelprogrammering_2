...
static HRESULT LoadTextureDataFromFile(const char* szFileName,BYTE** ppHeapData,
                                        DDS_HEADER** ppHeader,
...
static void GetSurfaceInfo( UINT width, UINT height, DXGI_FORMAT fmt, UINT* pNumBytes, UINT* pRowBytes, UINT* pNumRows )
{
    ...
    if( bc )
    {
        int numBlocksWide = 0;
        if( width > 0 )
            numBlocksWide = Math::Max( (UINT)1, width / 4 );
        int numBlocksHigh = 0;
        if( height > 0 )
            numBlocksHigh = Math::Max( (UINT)1, height / 4 );
...
static DXGI_FORMAT MAKE_SRGB( DXGI_FORMAT format )
{
    switch( format )
    {
        case DXGI_FORMAT_R8G8B8A8_TYPELESS:
...
//--------------------------------------------------------------------------------------
HRESULT CreateDDSTextureFromFile( ID3D10Device* pDev, const char* szFileName, ID3D10ShaderResourceView** ppSRV, bool bSRGB )
{
