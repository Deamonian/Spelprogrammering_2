...
void MessageSystem::Notify(IEvent* event) 
{
	...
	*/
	if(mMessageLists[(int)event->mType].empty()) { return; }

	auto it = mMessageLists[(int)event->mType].begin();
	auto it_end = mMessageLists[(int)event->mType].end();
...
// Register method already implemented in header

void MessageSystem::Unregister(EEventType type, void* object) 
{
	if(mMessageLists[(int)type].empty()) { return; }

	auto it_end = mMessageLists[(int)type].end();
	for(auto it = mMessageLists[(int)type].begin(); it != it_end; ++it) 
	{
		if((*it)->Compare(object)) 
		{
			delete (*it);
			mMessageLists[(int)type].erase(it);
			break;
		}
	}
}
...
void MessageSystem::Shut() 
{
	...
	{
		if(mMessageLists[i].empty()) { continue; }

		CallbackList::iterator it = mMessageLists[i].begin();
		CallbackList::iterator ie = mMessageLists[i].end();
