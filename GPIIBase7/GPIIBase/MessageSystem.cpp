...
void MessageSystem::Notify(IEvent *event) {
	...
	*/
	...
	auto it=m_message_lists[(int)event->m_type].begin();
	auto it_end=m_message_lists[(int)event->m_type].end();
	for( ;it!=it_end;++it) {
		(*(*it))(event);
	};
};
...
void MessageSystem::Unregister(EEventType type,void *object) {
	...
	auto it_end=m_message_lists[(int)type].end();
	for(auto it=m_message_lists[(int)type].begin();it!=it_end;++it) {
